...
# In this script we are training the convLSTM model straight forward, with not too many functions nested.
...
### END from train_all script


# straight forward code, without functions
        ...
        with torch.no_grad():
            ...
            #k, acc, QA = get_valid_record(valid_input, gt, net, factor_option=factor_option)
                # x_list, y_list = get_subsample_centroids(valid_input, img_size=256)
                    img_size = 256
                    h_total = valid_input.shape[-2]
                    w_total = valid_input.shape[-1]
                    ...
                    x_list, y_list = new_x_list, new_y_list
