...
def pre_prcessing(crop_img):
    crop_img_lulc = crop_img[:, 0, :, :]                                       # what are the dimensions? input is sub img from valid dataset
    temp_list = []
    for j in range(crop_img_lulc.shape[0]):
        temp = oh_code(crop_img_lulc[j], class_n=7)                            # what does 'oh' stand for? function below
    ...
    return oh_crop_img                                                         # how does output look like?   
...
def get_subsample_centroids(img, img_size=50): # img_size will be overwritten when used
    h_total = img.shape[-2] # height 1200
    w_total = img.shape[-1] # width 1200

    h_step = int(h_total // img_size * 1.5)                                    # why *1.5?
    w_step = int(w_total // img_size * 1.5)

    x_list = np.linspace(img_size//2, h_total-img_size//2, num = h_step) # Return evenly spaced numbers over a specified interval
    y_list = np.linspace(img_size//2, w_total -img_size//2, num= w_step)

    new_x_list = []
    new_y_list = []

    for i in x_list: # why create new lists? what is different?
        for j in y_list:
...
def get_valid_dataset(ori_data_dir):
    ori_data = np.load(ori_data_dir).transpose((1, 0, 2, 3))                   # what is the format of the data? 4 dims, dr.fact,year,width,height?
    # Down/up samples the input to the given scale_factor
    scaled_data = torch.nn.functional.interpolate(torch.from_numpy(ori_data),
                                                  scale_factor=(1 / 3, 1 / 3), # multiplier for spatial size, what does it do?
                                                  recompute_scale_factor=True)
    processed_ori_data = scaled_data.numpy()
    valid_input = processed_ori_data[1:5, :, :, :]                             # what are the dimensions? years?
    gt = processed_ori_data[-1, 0, :, :]                                       # what does gt refer to? ground truth? i.e. -1 last year, 0 land cover
    return valid_input, gt



def get_valid_record(valid_input, gt, net, device = torch.device('cuda'), factor_option = 'with_factors'):
    x_list, y_list = get_subsample_centroids(valid_input, img_size=256)        # why image size 256? centroids funct from above
    sub_img_list = []
...
def train_ensemble_ViT_FullValid(net = ensemble_ViT, device = torch.device('cuda'),
                  ...
                  pred_seq='forward', model_n='convgru', changemap = False):

    args = get_args()
    dataset_dir = "../data/train_valid/{}/{}/".format(pred_seq,'dataset_1')
    train_dir = dataset_dir + "train/"
    train_data = MyDataset(imgs_dir = train_dir + 'input/',masks_dir = train_dir +'target/')          # what are input and target? -> input: 2000-2015, target 2020?
    train_loader = DataLoader(dataset=train_data, batch_size=batch_size, shuffle=True, num_workers= 4)

    ori_data_dir = '../data/ori_data/211002_update_lulcmaps_withFactors_updated.npy'                  # what factors?

    valid_input, gt = get_valid_dataset(ori_data_dir)                          # function from above
    ...
    net.apply(weight_init)                                                     # what does weight_init do?
    ...
    for epoch in range(epochs):
        ...
        for i, (imgs, true_masks) in enumerate(train_loader):                  # what is the output of data_loader?
            imgs = imgs.to(device=device, dtype=torch.float32)
            imgs = Variable(imgs)                                              # what is Variable()? a torch function
            ...
            loss = criterion(masks_pred, true_masks[:,-1,:,:])                 # defined above as CrossEntropyLoss()
            ...
            true_masks_for_acc = true_masks[:,-1,:,:]                          # what is the input data shape?
        ...
        with torch.no_grad(): # Context-manager that disabled gradient calculation. saves computational power
            ...
            k, acc = get_valid_record(valid_input, gt, net, factor_option=factor_option) # function from above, valid_input from get_valid_dataset
