...
class ConvLSTM(nn.Module):
 ...
 # Make sure that both `kernel_size` and `hidden_dim` are lists having len == num_layers
        ...
        for i in range(0, self.num_layers):
            ...
            cell_list.append(nn.LSTM(input_size=cur_input_dim,
                                          hidden_size=self.hidden_dim[i],
                                          num_layers=self.kernel_size[i],
                                          bias=self.bias,
                                          batch_first = True))
...
class MV_LSTM(torch.nn.Module):
    ...
    def forward(self, x):        
        ...
        lstm_out1, self.hidden_layer0 = self.lstm1(x, self.hidden_layer0)
        ...
        x = lstm_out1.contiguous().view(batch_size,-1)
